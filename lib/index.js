...
var ImageToAscii = module.exports = function (options, callback) {
    ...
    Gm(options.path).size(function (err, size) {
        ...
        var tmpPath = "image-" + Math.random().toString(36) + ".png"
          , newSize = Util.computeSize(options.size, size, options.pxWidth)
          ;

        // Convert to png
        Gm(options.path).resize(newSize.width, newSize.height, options.aRatio === false ? "!" : undefined).write(tmpPath, function (err) {
            if (err) { return callback(err); }
